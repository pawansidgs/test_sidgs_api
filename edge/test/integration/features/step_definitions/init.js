...
var environment = config['test_sidgs_api'].env;
...
console.log('&test_sidgs_api api: [' +  domains[environment] + ', ' + config['test_sidgs_api'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['test_sidgs_api'].app, config['test_sidgs_api'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['test_sidgs_api'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
